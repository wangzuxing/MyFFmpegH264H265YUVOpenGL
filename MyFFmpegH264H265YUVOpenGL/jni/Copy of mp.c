 ...
 (JNIEnv *env, jclass clz, jstring h264_file)
{
   ...
   */
    ...
    {
	//Open output URL
	if (avio_open(&pFormatCtx->pb,out_file, AVIO_FLAG_READ_WRITE) < 0){//AVIO_FLAG_WRITE
	...
	video_st0 = avformat_new_stream(pFormatCtx, NULL);
 ...
 (JNIEnv *env, jclass clz)
{
	...
	while (ffmpeg_ce && av_read_frame(i_fmt_ctx, &pkt)>=0){
		while(pkt.size>0){
            ...
            int reference;\
			    	...
			    	//method 2
                    /*
			    	pgm_save2(ptrframe->data[0], ptrframe->linesize[0],
			    			pCodecCtx->width,
			    			pCodecCtx->height, temp_store);

			    	LOGI("22 Saving frame %d\n",frame_count);

			    	pgm_save2(ptrframe->data[1], ptrframe->linesize[1],
			    			pctx->width/2,
			    			pctx->height/2,
			    			temp_store+yuv_size);

			        pgm_save2(ptrframe->data[2], ptrframe->linesize[2],
			        		pctx->width/2,
			        		pctx->height/2,
			        		temp_store+yuv_size*5/4);

			        LOGI("33 Saving frame %d\n",frame_count);

			        //fwrite(ptrframe->data[0], 1, yuv_size*3/2, ptrfo);
			        fwrite(temp_store, 1, yuv_size*3/2, ptrfo);
                    */
	...
	if(ffmpeg_ce==0){
		...
		if (got_frame) {
			...
			pgm_save(ptrframe->data[0], ptrframe->linesize[0],
						    			pctx->width, pctx->height, NULL);
